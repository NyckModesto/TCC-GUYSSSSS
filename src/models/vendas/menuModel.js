...
module.exports = {
  formEditar: async (req, res) => {
    try {
      ...
      const prato = await menuModel.getById(pratoId, usuarioId);
      if (!prato) {
        return res.status(404).send("Prato não encontrado");
      }

      const tamanhos = await menuModel.buscarTamanhos(pratoId);

      res.render("menu/editar", { prato, tamanhos });
    } 
    catch (err) {
      console.error(err);
  ...
  salvarEdicao: async (req, res) => {
    try {
      ...
      await menuModel.updatePartial(pratoId, usuarioId, dados);

      // atualizar tamanhos, se enviados
      if (req.body.tamanhos) {
        const lista = JSON.parse(req.body.tamanhos);
        await menuModel.criarTamanhos(pratoId, lista);
      }
